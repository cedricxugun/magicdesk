...
var world:Node3D
var camera:Camera3D
var out:="res://../review/I_refinement/part_a_mouth/shutter_r2/finish_filter/"
...
func run()->void:
    ...
    for quality in [2,3,4,5]:
        root.scaling_3d_scale=1.;root.use_taa=false
        RenderingServer.positional_soft_shadow_filter_set_quality(quality)
        var label:="filter_%d"%quality
        ...
        for frame in range(150):
            ...
            previous=now
        cadence[label]=times
    FileAccess.open(out+"audit.json",FileAccess.WRITE).store_string(JSON.stringify({"source_sha256":spec.source_sha256,"component_sha256":spec.component_sha256,"filter_candidates":[2,3,4,5],"frame_cadence_ms":cadence,"scope":"Native Metal at 1200x1000, Low/Medium/High/Ultra soft shadows, 4x MSAA, no TAA, vsync disabled during this bounded cadence test. Measures end-to-end frame cadence, not isolated GPU time or whole App release performance."},"  "))
