extends SceneTree
var host:Node3D
var out:="res://../review/I_refinement/opening_r15/runtime/"
...
func run()->void:
    ...
    var path:="res://assets/collection/components/I_opening_r15.glb";var asset:Node3D=load(path).instantiate();host.add_child(asset)
    var driver:RefCounted=load("res://collection/i_opening_layout_r15.gd").new();driver.setup(asset)
    var source:Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://../review/I_refinement/opening_r15/source_poses.json"));var error:=0.;var samples:Array=[]
