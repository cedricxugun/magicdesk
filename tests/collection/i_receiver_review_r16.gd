extends SceneTree
var host:Node3D
var out:="res://../review/I_refinement/receiver_r16/runtime/"
...
func run()->void:
    ...
    var path:="res://assets/collection/components/I_receiver_r16.glb";var asset:Node3D=load(path).instantiate();host.add_child(asset)
    var driver:RefCounted=load("res://collection/i_opening_layout_r15.gd").new();driver.setup(asset,"res://assets/collection/i_receiver_rig_r16.json")
    var source:Dictionary=JSON.parse_string(FileAccess.get_file_as_string("res://../review/I_refinement/receiver_r16/source_poses.json"));var error:=0.;var samples:Array=[]
    ...
    for frame in range(1,362):
        ...
        if source.has(str(frame)):
            for name in source[str(frame)]:
                ...
                var p:=Vector3(row.p[0],row.p[1],row.p[2]);var q:=Quaternion(row.q[0],row.q[1],row.q[2],row.q[3]);var scale:=Vector3(row.s[0],row.s[1],row.s[2]);var rot:=Basis(q);var expected:=Transform3D(Basis(rot.x*scale.x,rot.y*scale.y,rot.z*scale.z),p)
    ...
    var result:={"source_pose_match":error<.00002,"max_pose_error":error,"samples":samples,"component_sha256":FileAccess.get_sha256(path),"scope":"Actual Metal 12-second kinematic layout forward/reverse with sampled source transforms. Guided release/support envelopes are provisional. No collision, art, native controls or music acceptance."}
