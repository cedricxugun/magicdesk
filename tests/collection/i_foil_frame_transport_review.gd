...
var world:Node3D
var camera:Camera3D
var out:="res://../review/I_refinement/part_a_mouth/shutter_r2/foil_frame/transport_float_r2/"
...
func run()->void:
    ...
    for group in spec.tongues:
        for name in group.mesh_names:
            ...
            foils.append(node);original_shaders.append(node.material_override)
            var candidate:ShaderMaterial=node.material_override.duplicate()
            candidate.shader=load("res://review/i_tongue_frame_transport.gdshader");corrected_shaders.append(candidate)
    ...
    for opening in [.25,.5]:
        ...
        for mode in ["legacy","corrected"]:
            for i in range(foils.size()):
                var feed:float=original_shaders[i].get_shader_parameter("feed")
