...
var world:Node3D
var camera:Camera3D
var out:="res://../review/I_refinement/part_a_mouth/shutter_r2/echo_r2/audiovisual/"
...
func run()->void:
    ...
    var presentation=load("res://collection/i_echo_presentation.gd").new();world.add_child(presentation);presentation.setup(asset,true)
    ...
    FileAccess.open(out+"take.json",FileAccess.WRITE).store_string(JSON.stringify({"source_sha256":spec.source_sha256,"component_sha256":spec.component_sha256,"frames":360,"fps":60,"samples":samples,"events":emitted,"scope":"Actual Godot MovieWriter visual/audio mix, complete local mouth with authored wavefront/mask and original offline cues. Not a final full-conch/music or native listening acceptance."},"  "))
    print("I_ECHO_AUDIOVISUAL_FINISHED");world.queue_free();await process_frame;quit()
