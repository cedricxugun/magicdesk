...
var body:Node3D
var mouth:Node3D
var out:="res://../review/I_refinement/nautilus_reset_r82/network_r90/built_r5/native_default/"
var duration:=35.
var report_path:="res://../review/I_refinement/nautilus_reset_r82/network_r90/built_r5/build.json"
var polished:=false
...
func run()->void:
    ...
    # Warm a hidden offscreen viewport, including active optics/material variants.
    var warm:=SubViewport.new();warm.size=Vector2i(256,256);warm.msaa_3d=root.msaa_3d;warm.world_3d=world.get_world_3d();warm.render_target_update_mode=SubViewport.UPDATE_ALWAYS;root.add_child(warm)
    var warm_camera:=Camera3D.new();warm.add_child(warm_camera);warm_camera.transform=camera.transform;warm_camera.projection=camera.projection;warm_camera.size=camera.size;warm_camera.current=true
    driver.set_opening(1.);rig.tongues.set_opening(1.)
    for i in range(12):
        music.staff.set_score_position(1,4.,1.);music.staff.set_music_response(.5,true,true)
        if chambers:chambers.update(Vector3(.6,.6,.6),float(i)*.1,true,1.,1./30.)
        RenderingServer.force_draw(false,0.);await process_frame
    driver.set_opening(0.);rig.tongues.set_opening(0.);music.staff.set_score_position(1,0.,0.);music.staff.set_music_response(0.,false,false)
    for spill in optical_spill:spill.light_energy=0.
    if chambers:chambers.update(Vector3.ZERO,0.,false,0.,1.)
    warm.queue_free();await process_frame;camera.current=true
    var screen_rect:=DisplayServer.screen_get_usable_rect()
    root.position=screen_rect.position+Vector2i(maxi(20,(screen_rect.size.x-root.size.x)/2),maxi(20,(screen_rect.size.y-root.size.y)/2))
    root.grab_focus()
    var samples:Array=[];var captures:=[0.,4.,8.,15.,30.]
    var next_capture:=0;var elapsed:=0.;var last:=Time.get_ticks_usec()
    sequence.music_request();music.toggle()
    while elapsed<duration:
        await process_frame
        var now:=Time.get_ticks_usec();var dt:=minf(.1,float(now-last)/1000000.);last=now;elapsed+=dt
        sequence.tick(dt);rig.tick(dt);music.transport._process(0.);music.tick(dt)
        if chambers:chambers.update(music.transport.chamber_bands(),music.transport.position_seconds() if chambers.wants_work_clock() else music.transport.local_clock,music.transport.state=="playing",sequence.shell_open,dt)
        update_optical_spill()
        RenderingServer.force_draw(false,dt)
        if next_capture<captures.size() and elapsed>=captures[next_capture]:
            root.get_texture().get_image().save_png(out+"state_%02d.png"%next_capture)
            samples.append({"elapsed":elapsed,"sequence":sequence.snapshot(),"music":music.status.duplicate(true),"driver":driver.snapshot(),"chambers":chambers.status.duplicate(true) if chambers else {}})
            print("R83_NATIVE_CAPTURE ",next_capture," ",elapsed," ",music.transport.state)
            next_capture+=1
    FileAccess.open(out+"render_review.json",FileAccess.WRITE).store_string(JSON.stringify({"body_sha256":spec.component_sha256,"core_sha256":spec.core_component_sha256,"optics_sha256":music.staff.layout.component_sha256,"polished_studio":polished,"temporal_polish":temporal_polish,"rear_review":rear_review,"samples":samples,"scope":"Actual Godot combined renderer and real audio-clock first-movement excerpt, fixed camera and authored mechanism. Explicit draw; no native performance or full art/fit acceptance."},"  "))
    music.stop(true);sequence.set_open(false)
    for i in range(360):
        sequence.tick(1./30.);rig.tick(1./30.);music.tick(1./30.)
    ...
    print("R83_NATIVE_REVIEW_DONE");quit()
