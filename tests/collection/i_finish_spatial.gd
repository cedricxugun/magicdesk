...
var world:Node3D
var camera:Camera3D
var out:="res://../review/I_refinement/part_a_mouth/shutter_r2/finish_spatial/"
...
func run()->void:
    ...
    for scale in [1.,1.25,1.5]:
        root.scaling_3d_scale=scale;root.use_taa=false
        var label:="scale_%03d"%int(scale*100.)
        ...
        await capture(label+"_whole")
        var times:Array=[];var previous:=Time.get_ticks_usec()
        for frame in range(150):
            driver.set_opening(.35+.15*sin(float(frame)*.024))
            await RenderingServer.frame_post_draw
            var now:=Time.get_ticks_usec()
            if frame>=30:times.append(float(now-previous)/1000.)
            previous=now
        cadence[label]=times
    FileAccess.open(out+"audit.json",FileAccess.WRITE).store_string(JSON.stringify({"source_sha256":spec.source_sha256,"component_sha256":spec.component_sha256,"render_scale_candidates":[1.,1.25,1.5],"frame_cadence_ms":cadence,"scope":"Native Metal at 1200x1000, ULTRA soft shadows, 4x MSAA, no TAA, vsync disabled during this bounded cadence test. Measures end-to-end frame cadence, not isolated GPU time or whole App release performance."},"  "))
    print("I_FINISH_SPATIAL_FINISHED");world.queue_free();await process_frame;quit()
