...
func run()->void:
	...
	var directory:="coupling_repair_r76" if baseline else "base_dock_r79"
	...
	out=ProjectSettings.globalize_path("res://../review/I_refinement/nautilus_r1/base_dock_r79/"+("baseline/" if baseline else "views/")).simplify_path()+"/"
