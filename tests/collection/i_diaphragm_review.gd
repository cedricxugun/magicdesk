...
var world:Node3D
var camera:Camera3D
var out:="res://../review/I_refinement/part_a_mouth/shutter_r2/diaphragm/runtime/"
...
func run()->void:
    ...
    for view in ["front","rear"]:
        camera.position=Vector3(-1.4,-2.1,-.1) if view=="front" else Vector3(1.15,2.1,-.3)
        camera.look_at(Vector3(0,.27,0),Vector3.FORWARD)
        for value in [0.,.006,-.003]:
            response.set_displacement(value);await capture(view+"_%d"%int(value*10000.))
    # Isolated suspension diagnostic, never presented as normal operating state.
    var keep:Dictionary={}
    ...
    camera.position=Vector3(.95,1.2,-.1);camera.look_at(Vector3(.05,.4,0),Vector3.FORWARD)
    for value in [0.,.006,-.003]:
        response.set_displacement(value);await capture("suspension_diagnostic_%d"%int(value*10000.))
    FileAccess.open(out+"review.json",FileAccess.WRITE).store_string(JSON.stringify({"source_sha256":spec.source_sha256,"component_sha256":spec.component_sha256,"studio":receipt,"scope":"Actual Metal source-backed suspension morph views at rest/pressure/rebound. Isolated view hides guard/case for diagnosis, not a service animation or full-device acceptance."},"  "))
    print("I_DIAPHRAGM_REVIEW_FINISHED");world.queue_free();await process_frame;quit()
