extends SceneTree
const OUT:="res://../review/G_optical_curator/observatory_materials/"
var host:Node3D
var module:Node3D
var service:Node3D
...
func run()->void:
	...
	var root_node:Node3D=player.content[0].node
	...
	for mesh in module.meshes:
		for i in range(mesh.mesh.get_surface_count()):
			...
			if not mat.has_meta("observatory_finish"):continue
			changed+=1;families[mat.get_meta("observatory_finish")]=true
			if not root_node.is_ancestor_of(mesh):outside+=1
			if mat.get_instance_id() in cache_ids:shared+=1
	var result:={"passed":changed>0 and families.size()==6 and outside==0 and shared==0,"finished_surfaces":changed,"families":families.keys(),"outside_observatory":outside,"shared_cache_mutations":shared,"scope":"Inspect actual materials after collection setup; verify finish copies do not recolor other G content or shared resources"}
	FileAccess.open(OUT+"isolation_qa.json",FileAccess.WRITE).store_string(JSON.stringify(result,"  "));print("OBSERVATORY_MATERIAL_QA ",JSON.stringify(result))
