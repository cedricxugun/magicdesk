...
func run()->void:
	...
	var out:="res://../review/I_refinement/nautilus_r1/service_r63/"+("stage03_wide/" if wide else "stage03/")
	...
	driver.request(true)
	for i in range(420):driver.tick(1./60.)
	...
	driver.release();driver=null;body.queue_free();await process_frame;await process_frame;print("SERVICE03_RUNTIME true");quit()
