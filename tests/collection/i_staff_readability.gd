...
func run()->void:
    ...
    var staff=load("res://collection/i_moonlight_staff.gd").new();world.add_child(staff);staff.setup(asset,str(spec.component_sha256))
    driver.set_opening(1.)
    var variant:="baseline"
    for argument in OS.get_cmdline_user_args():
        if argument.begins_with("--variant="):variant=argument.trim_prefix("--variant=")
    out="res://../review/I_refinement/moonlight/current_mouth/readability/"+variant+"/";DirAccess.make_dir_recursive_absolute(out)
    var samples:Array=[]
    for view in ["desktop","front"]:
        camera.position=Vector3(-2.55,-2.4,-.82) if view=="desktop" else Vector3(-1.,-3.4,-.6)
        camera.look_at(Vector3(0,.03,0),Vector3.FORWARD)
        for sample in [[1,32.],[2,0.],[3,640.]]:
            for frame in range(60):
                staff.set_score_position(sample[0],sample[1],1.)
                await RenderingServer.frame_post_draw
                if staff.status.ready:break
            assert(staff.status.ready)
            await capture(view+"_m%d_q%d"%[sample[0],int(sample[1])]);samples.append(staff.status.duplicate(true))
    FileAccess.open(out+"review.json",FileAccess.WRITE).store_string(JSON.stringify({"source_sha256":spec.source_sha256,"component_sha256":spec.component_sha256,"optics_sha256":staff.layout.component_sha256,"width":staff.layout.width,"height":staff.layout.height,"samples":samples,"scope":"Fixed cameras, same actual score locations, 1200x1000 comparison. Local readability evidence only, not whole-conch or musical synchronization acceptance."},"  "))
    print("I_STAFF_READABILITY_FINISHED");world.queue_free();await process_frame;quit()
