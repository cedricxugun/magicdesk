...
func run()->void:
	...
	var directory:="coupling_repair_r76" if baseline else "base_dock_r81"
	...
	out=ProjectSettings.globalize_path("res://../review/I_refinement/nautilus_r1/base_dock_r81/"+("baseline/" if baseline else "views/")).simplify_path()+"/"
	...
	if OS.get_cmdline_user_args().has("--motion"):
		...
		var qa_only:=OS.get_cmdline_user_args().has("--qa-only")
		var witnesses:Dictionary={};var local_points:Array=[];var witness_error:=0.;var witness_count:=0
		var witness_path:="res://../review/I_refinement/nautilus_r1/base_dock_r81/opening_witnesses.json"
		if qa_only:
			var measured:Dictionary=JSON.parse_string(FileAccess.get_file_as_string(witness_path))
			assert(measured.body_source_sha256==spec.source_sha256)
			local_points=measured.local_points
			for sample in measured.samples:witnesses[int(sample.frame)]=sample.points
		...
		for frame in range(420):
			...
			if witnesses.has(frame):
				for name in witnesses[frame]:
					var node:Node3D=body.find_child(name,true,false);assert(node!=null)
					for k in range(local_points.size()):
						var local_point:Array=local_points[k];var expected:Array=witnesses[frame][name][k]
						witness_error=maxf(witness_error,node.to_global(Vector3(local_point[0],local_point[1],local_point[2])).distance_to(Vector3(expected[0],expected[1],expected[2])));witness_count+=1
			if not qa_only:
			...
			if frame%90==0:print("R81_FRAME ",frame)
		if qa_only and (witness_error>.00001 or witness_count!=1740):push_error("Saved animation mismatch: "+str(witness_error)+" "+str(witness_count));quit(6);return
		FileAccess.open(out+("qa_receipt.json" if qa_only else "receipt.json"),FileAccess.WRITE).store_string(JSON.stringify({"source_sha256":spec.source_sha256,"component_sha256":spec.component_sha256,"frames":0 if qa_only else 420,"fps":30,"fixed_dock_meshes":homes.size(),"fixed_dock_transform_check":true,"actual_saved_blender_points":witness_count,"max_blender_point_error":witness_error,"samples":samples,"scope":"Offline real-source closed/open/closed demonstration at normal playback speed. QA compares actual saved Blender animation points. No service release, audio or native performance claim."},"  "))
