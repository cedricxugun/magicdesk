...
func make()->Node3D:
	...
	module.play.g_instrument=load("res://collection/optical_curator.gd").new();module.play.g_instrument.setup(module);return module
func advance(module:Node3D,dt:float)->void:
	...
	var tcp:Vector3=p.wrist_virtual+Basis(p.wrist_q)*Vector3.UP*float(data.record_player.carrier_offset)+Vector3.UP*.35;var shoulder:Vector3=module.v3(data.record_player.shoulder);var reach:float=tcp.distance_to(shoulder)
	if reach>float(data.record_player.lengths[0])+float(data.record_player.lengths[1])+.00001 or reach<absf(float(data.record_player.lengths[0])-float(data.record_player.lengths[1]))-.00001:violations.append("arm outside analytic reach")
...
func run()->void:
	data=JSON.parse_string(FileAccess.get_file_as_string("res://assets/collection/models/G_optical_curator.json"))
	...
	check("enlarged_same_disc_contract",is_equal_approx(float(data.record_player.record_radius),.42) and is_equal_approx(float(data.record_player.platter_origin[1]),.924))
	var ok:bool=checks.all(func(c):return c.passed)
	FileAccess.open("res://../review/G_optical_curator/state_qa.json",FileAccess.WRITE).store_string(JSON.stringify({"all_passed":ok,"checks":checks,"stages":stages.keys()},"  "));quit(0 if ok else 2)
