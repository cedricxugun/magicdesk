...
stage=Path(tempfile.mkdtemp(prefix='butterfly_mac_',dir=stage_parent));app=stage/'app'
...
g['scene']='res://assets/collection/models/G_optical_curator_butterfly_candidate.glb';g['metadata']='res://assets/collection/models/G_optical_curator_butterfly_candidate.json';g['development_status']='G1 isolated native review; not final all-model acceptance';path.write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
path=app/'project.godot';path.write_text(path.read_text().replace('config/name="MagicDesk"','config/name="MagicDesk G1 Review"'))
path=app/'export_presets.cfg';path.write_text(path.read_text().replace('application/bundle_identifier="com.cedricxugun.magicdesk"','application/bundle_identifier="com.cedricxugun.magicdesk.g1review"'))
dest=ROOT/'dist/macos-review/MagicDesk-G1-review.app';dest.parent.mkdir(exist_ok=True)
subprocess.run([GODOT,'--headless','--editor','--path',str(app),'--import'],check=True)
...
out=ROOT/'review/G_optical_curator/butterfly_r2/native';out.mkdir(exist_ok=True);(out/'build.json').write_text(json.dumps(report,indent=2)+'\n');print('BUTTERFLY_NATIVE_BUILD',json.dumps(report),flush=True)
