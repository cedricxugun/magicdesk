...
OUT = ROOT / 'review/I_refinement/nautilus_r1/port_envelope_r67/smooth_r3'
...
heights = points[5*192:12*192,2].reshape((7,192)).copy()
expanded = heights.copy()
for radial in range(-2,3):
    rows = np.clip(np.arange(7)+radial,0,6)
...
points[5*192:12*192,2]=smoothed.reshape(-1)
