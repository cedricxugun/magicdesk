...
OUT=ROOT/'review/I_refinement/conch_r13';OUT.mkdir(parents=True,exist_ok=True)
TARGET=ROOT/'blender/collection/I_conch_r13.blend';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
if TARGET.exists():
...
assert sha(ROOT/'blender/collection/I_conch_r12.blend')=='f44349b31653c7514141b58e710025ad67679e6e154e5a9c0acd7d31856ef581','R12 input changed; re-evaluate before applying old form delta'
...
def band_limits(index,theta):
 f=theta/math.tau;lo=max(0.,(index-1+f)/5.+.0018);hi=min(1.,(index+f)/5.-.0018)
 if hi<lo:lo=hi=max(0.,min(1.,(lo+hi)*.5))
 return lo,hi
def band_theta(index,fraction):
 start=math.tau*5.*(.0018+.001) if index==0 else .0025
 end=math.tau*(1.-5.*(.0018+.001)) if index==5 else math.tau-.0025
 return start+(end-start)*fraction
def band_surface(index,u,theta,inside=False):
 lo,hi=band_limits(index,theta);q=lo+(hi-lo)*u;a=math.pi/2-theta
 rr=radius(q)*(1.+.095*math.sin(math.pi*u)*min(1.,(hi-lo)*5.));thickness=min(.030,rr*.30)
 if inside:rr-=thickness
 return center(q)+Vector((rr*math.cos(a),rr*.80*math.sin(a),0))
def shell_section(index):
 nu=32;nv=192;stride=nv+1;verts=[];faces=[];uv=[]
 parent=b.empty('IC11_SpiralPlate'+str(index),body)
 for inside in [False,True]:
  for j in range(nu+1):
   for k in range(nv+1):
    theta=band_theta(index,k/nv);verts.append(band_surface(index,j/nu,theta,inside));uv.append((k/nv,j/nu))
 size=(nu+1)*stride
 for layer in [0,1]:
  for j in range(nu):
   for k in range(nv):
    a=layer*size+j*stride+k;face=(a,a+1,a+stride+1,a+stride);faces.append(face if layer==0 else face[::-1])
 boundary=list(range(stride))+[j*stride+nv for j in range(1,nu+1)]+[nu*stride+k for k in range(nv-1,-1,-1)]+[j*stride for j in range(nu-1,0,-1)]
 for a,c in zip(boundary,boundary[1:]+boundary[:1]):faces.append((a,c,c+size,a+size))
 o=b.fast['fast_instance'](b.name('PorcelainHelix'+str(index)),verts,faces,'ConchPorcelain',parent,(0,0,0),smooth_faces=True,uv=uv);apply(o)
...
shells=[]
for i in range(6):
 parent,o=shell_section(i);boolean(o,cut)
 bevel=o.modifiers.new('Porcelain moulded edge','BEVEL');bevel.width=.004;bevel.segments=3;apply(o)
 bm=bmesh.new();bm.from_mesh(o.data)
 for edge in bm.edges:
  if edge.is_manifold and edge.calc_face_angle()>math.radians(35):edge.smooth=False
 bm.to_mesh(o.data);bm.free()
 shells.append({'node':parent.name,'mesh':o.name,'helical_band':i,'pitch_normalized':.2})
 for u in [0.,1.]:
  lip=b.tube('HelicalNickelEdge',[band_surface(i,u,band_theta(i,k/192)) for k in range(193)],.008,'ConchNickel',parent)
 ...
 for theta in [.35,2.1,3.8,5.35]:
  lo,hi=band_limits(i,theta)
  if hi-lo<.015:continue
  q=lo+(hi-lo)*.90;a=math.pi/2-theta;p=band_surface(i,.90,theta);normal=Vector((math.cos(a),math.sin(a)/.8,.18)).normalized();scale=min(1.,radius(q)/.30)
  local=mouth.matrix_world.inverted()@(body.matrix_world@p)
  if math.hypot(local.x,local.y)<.84 and local.z<.45:continue
  b.cube('ClampSaddle',(.035*scale,.063*scale,.014*scale),'ConchNickel',parent,p,.004*scale,normal.to_track_quat('Z','Y'))
...
bpy.context.view_layer.objects.active=b.root;component=ROOT/'app/assets/collection/components/I_conch_r13.glb'
