...
r={'source_sha256':s['source_sha256'],'source':s['source'],'mesh':o.name,'plane_z':1.698,'opposing_original_plane':1.702,'outward_sign':1,'gap':.004,'cap_polygon_count':len(selected),'cap_triangle_count':len(triangles),'loops':loops,'triangles':triangles,'mesh_matrix_world':[list(r)for r in world],'scope':'Actual horizontal cut-face footprint. Read-only extraction; not a finished rim or clearance proof.'};(ROOT/'review/I_refinement/nautilus_r1/cap_lower_profile_r55/profile.json').write_text(json.dumps(r,indent=2)+'\n');print('R52_CAP_PROFILE',{'polygons':len(selected),'triangles':len(triangles),'loops':[{'n':len(l['source_vertices']),'area':l['signed_area_xy'],'length':l['length'],'minimum_edge':l['minimum_edge_length']}for l in loops]},flush=True)
