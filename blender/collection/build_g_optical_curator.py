"""Optical curator: independent AI face and optical carrier, same physical media
...
# Neutral runtime mesh avoids loading a newer Windows .blend destructively
# into this machine's Blender 5.1. Original source remains untouched.
bpy.ops.wm.read_factory_settings(use_empty=True)
bpy.ops.import_scene.gltf(filepath=str(ROOT/'app/assets/collection/models/G_record_player.glb'))
...
scene=bpy.context.scene;scene.name='G_Optical_Curator';scene.frame_set(1)
b=Builder.__new__(Builder);b.id='GA';b.title='Sixfold archive player';b.scene=scene
...
def joint(parent,loc,radius=.095):
    ...
    for side in [-1,1]:
        ...
        b.sphere('ConvexRubyTorqueLens',radius*.58,'PlayerRuby',parent,p+Vector((side*.008,0,0)),(.17,1.,1.))
...
revolve('CeramicStand',[(.665,.20,.21),(.73,.235,.235),(.83,.20,.20),(.96,.15,.15),(1.30,.13,.13),(1.55,.12,.12),(1.67,.17,.17),(1.745,.12,.12)],'PlayerPorcelain',stand,(0,.20,0))
for z,r in [(.704,.189),(.775,.146),(1.43,.105),(1.505,.130)]:b.torus('StandGoldCollar',r,.013,'PlayerGold',stand,(0,.20,z))
...
b.torus('ShoulderYawRace',.125,.011,'PlayerGold',stand,(0,.20,1.747))
...
mag_part=b.part('Magazine',(0,0,.25),.70);mag=b.empty('GR_MagazineRotor',mag_part,(0,0,0))
# Two stepped three-disc banks leave the reader and printing volume clear.
for side in [-1,1]:
    path=[(side*.34,.70,.68),(side*.95,.65,.88),(side*.96,-.28,.79)]
    b.tube('MagazineSpine',path,.045,'PlayerNickel',mag)
    b.tube('MagazineGildedConduit',[(x,y,z+.055) for x,y,z in path],.008,'PlayerGold',mag)
cup_names=[];disc_names=[];slot_poses=[]
for i in range(6):
    side=-1 if i<3 else 1;depth=i%3;a=i*math.tau/6
    center=Vector((side*(1.08-.15*depth),.65-.45*depth,1.60-.20*depth))
    normal=Vector((-side*.50,-.81,.30)).normalized()
    rotation=Vector((0,0,1)).rotation_difference(normal)
    home=Matrix.Translation(center)@rotation.to_matrix().to_4x4()
    b.beam('CradleSupport',Vector((center.x,center.y,.73)),center+rotation@Vector((0,-.38,-.14)),.035,'PlayerNickel',mag)
    b.cyl('CradleDeckFoot',.069,.055,'PlayerGold',mag,(center.x,center.y,.708),n=48)
    ...
    disc=b.empty('GR_Record'+str(i),dp);disc.matrix_basis=home;disc_names.append(disc.name);disc.scale=(1.68,1.68,1.68)
...
upper_part=b.part('UpperArm',(-.35,.20,.48),.55);upper=b.empty('GR_UpperArm',upper_part);joint(upper,(0,0,0),.135);bone(upper,'UpperPorcelainLink',.88,.116)
fore_part=b.part('Forearm',(.35,.10,.76),.42);fore=b.empty('GR_Forearm',fore_part);joint(fore,(0,0,0),.118);bone(fore,'ForearmPorcelainLink',.98,.103)
wrist_part=b.part('Wrist',(0,-.25,.99),.15);wrist=b.empty('GR_Wrist',wrist_part)
from optical_curator_parts import make_head,clevis
head_spec=make_head(b,wrist)
shoulder_fork=clevis(b,b.upper,'GA_ShoulderFork',.143,.088,.17);shoulder_fork.location=(0,.20,1.75)
elbow_fork=clevis(b,upper,'GA_ElbowFork',.139,.075,.15);elbow_fork.location=(0,0,.88)
head_spec['shoulder_fork']=shoulder_fork.name
joint(fore,(0,0,.98),.070)
finger_rigs=[]

# Distinct small reader (0.80 m), on the front of the authoritative 2.74 m base.
platter_part=b.part('ReaderPlatter',(0,-.50,.25),.4)
revolve('SmallReaderHousing',[(.73,.36,.36),(.77,.445,.445),(.835,.455,.455),(.89,.455,.455),(.915,.438,.438)],'PlayerNickel',platter_part,(0,-.72,0))
for z,r in [(.78,.448),(.893,.455),(.918,.438)]:b.torus('SmallReaderGoldLip',r,.010,'PlayerGold',platter_part,(0,-.72,z))
platter=b.empty('GR_PlatterPivot',platter_part,(0,-.72,.924))
b.cyl('SmallPlatterMat',.433,.012,'PlayerRubber',platter,n=120)
b.torus('PlatterEdgeHighlight',.431,.003,'PlayerNickel',platter,(0,0,.007))
...
socket=revolve('TonearmLiftSocket',[(.69,.071,.071),(.724,.078,.078),(.83,.052,.052),(.935,.050,.050)],'PlayerGold',tone_base,(.68,-.62,0))
cutter=b.cyl('TemporaryToneBore',.030,.39,'Black',tone_base,(.68,-.62,.85),n=64);bpy.context.view_layer.update();bpy.context.view_layer.objects.active=socket
...
b.cyl('ToneLiftRubyBand',.054,.033,'PlayerRuby',tone_base,(.68,-.62,.85),n=56)
tone_part=b.part('OpticalTonearm',(.38,-.15,.60),.2);tone=b.empty('GR_Tonearm',tone_part,(.68,-.62,1.10))
b.cyl('TonearmLiftPiston',.027,.34,'PlayerNickel',tone,(0,0,-.145),n=48)
b.cyl('TonearmUpperBearing',.066,.055,'PlayerGold',tone,(0,0,.016),n=64)
b.torus('TonearmBearingSeal',.062,.006,'Black',tone,(0,0,.045))
...
b.cyl('OpticalHead',.060,.072,'PlayerGold',tone,(.52,0,-.012),n=64)
b.sphere('ProbeArmUnion',.051,'PlayerPorcelain',tone,(.498,0,.025),(1.3,1.,.65))
b.cyl('RubyOptic',.039,.008,'PlayerRuby',tone,(.52,0,-.051),n=64)
b.torus('OpticRim',.045,.006,'PlayerNickel',tone,(.52,0,-.054))
needle=b.empty('GR_ScanPoint',tone,(.52,0,-.058))
...
def arm(tcp):
    S=Vector((0,.20,1.75));W=Vector(tcp);horizontal=Vector((W.x-S.x,W.y-S.y,0));distance=horizontal.length;direction=horizontal.normalized();height=W.z-S.z;co=max(-1,min(1,(distance*distance+height*height-.88**2-.98**2)/(2*.88*.98)));q2=-math.acos(co);q1=math.atan2(height,distance)-math.atan2(.98*math.sin(q2),.88+.98*math.cos(q2));E=S+direction*(.88*math.cos(q1))+Vector((0,0,.88*math.sin(q1)));aim(upper,S,E);aim(fore,E,W);wrist.location=W
arm((-.56,.18,2.65))
direction=Vector((-.56,-.02,0)).normalized();axis=Vector((-direction.y,direction.x,0))
shoulder_fork.rotation_euler.z=math.atan2(axis.y,axis.x)
bpy.data.objects[head_spec['wrist_yoke']].matrix_basis=fore.matrix_basis.to_3x3().to_4x4()
...
from observatory_asset import replace_observatory
replace_observatory(catalog,print_root,b.col)
from curator_wire import extract
for item in catalog:
    if not item.get('build_guides'):item['structure_edges']=extract(bpy.data.objects[item['root']])
parts=[{'name':p['obj'].name,'home':pose(p['obj'].matrix_basis),'offset':p['offset'],'stage':p['stage']} for p in b.parts]
metadata={'id':'G','title':'光学馆员','root':b.root.name,'upper':b.upper.name,'parts':parts,'controls':[],'motions':[],'sockets':{},'qa_shells':[],'part_count':len(parts),'base_diameter':2.74,'includes_base':False,'source_blend':'blender/collection/G_optical_curator.blend','display_calibration':{'scale':1.0,'fixed_mount_height':.615,'base_scaled':False},'g_archive':{'contents':catalog,'specimen_scale':1.0},'record_player':{'magazine':mag.name,'cradles':cup_names,'records':disc_names,'slots':slot_poses,'upper_arm':upper.name,'forearm':fore.name,'wrist':wrist.name,'fingers':finger_rigs,'platter':platter.name,'print_root':print_root.name,'tonearm':tone.name,'scan_point':needle.name,'shoulder':[0,1.75,-.20],'lengths':[.88,.98],'record_radius':.42,'record_scale':1.68,'record_thickness':.02352,'platter_origin':[0,.924,.72],'disc_height':.01976,'magazine_origin':[0,0,0],'tone_origin':[.68,1.030,.62],'tone_length':.52,'source_atlas':'res://assets/collection/art/G_AI/optical_field_atlas.png','seconds_per_turn':60,'grip_offset':.34,'design':'production/G_record_player/r3_fx/README.md'}}
metadata['optical_curator']=head_spec
metadata['record_player'].update({'park':[-.56,2.02,-.18],'place':[0,.94376,.72],'take':[0,1.46,.74],'lift':[0,2.35,.74],'level':[0,2.35,.74],'above':[0,2.35,.72],'carrier_offset':.28,'beam_gap':.18,'read_radii':[.35,.105],'cradle_scale':1.68})
from curator_service_layout import arrange
arrange(parts,metadata['record_player'],head_spec)
seen=set()
...
out=ROOT/'app/assets/collection/models/G_optical_curator.glb';out.with_suffix('.json').write_text(json.dumps(metadata,separators=(',',':')),encoding='utf-8')
...
for obj in added:bpy.data.objects.remove(obj,do_unlink=True)
scene.render.fps=30;scene.frame_end=1800;scene.timeline_markers.clear();scene.timeline_markers.new('Optical curator - runtime bake pending',frame=1)
...
print('OPTICAL_CURATOR_BUILT',len(parts),'parts; six persistent .84 media; separate .90 reader; independent AI head and optical carrier',flush=True)
