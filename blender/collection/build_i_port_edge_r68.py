...
s=json.loads((ROOT/'review/I_refinement/nautilus_r1/port_envelope_r67/smooth_r3/build.json').read_text());sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest();assert sha(ROOT/s['source'])==s['source_sha256'];bpy.ops.wm.open_mainfile(filepath=str(ROOT/s['source']));bpy.context.scene.frame_set(1);bpy.context.view_layer.update()
...
M=o.matrix_world.copy();bm=bmesh.new();bm.from_mesh(o.data);bm.verts.ensure_lookup_table();bm.verts.index_update();bm.edges.ensure_lookup_table()
assert len(bm.verts)==4224
edges=[e for e in bm.edges if len({v.index//192 for v in e.verts})==1 and next(iter(e.verts)).index//192 in [11,12]]
assert len(edges)==384
before={v.index:v.co.copy()for v in bm.verts}
result=bmesh.ops.bevel(bm,geom=edges,offset=.00012,segments=5,affect='EDGES',clamp_overlap=True,loop_slide=True,profile=.5)
bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces))
assert all(e.is_manifold for e in bm.edges)and all(v.is_manifold for v in bm.verts)
volume=bm.calc_volume(signed=True);bm.to_mesh(o.data);bm.free();o.data.update()
for face in o.data.polygons:face.use_smooth=True
solution={'rounded_rings':[11,12],'original_selected_edges':384,'requested_bevel_offset':.00012,'bevel_segments':5,'scope':'Physical bevel only on first liner outer skirt boundaries. Source manifold/self and local clearance checked below; not full sequence or art acceptance.'}
...
(OUT/'build_check.json').write_text(json.dumps(probe,indent=2)+'\n');print('R68_CHECK',json.dumps(probe),flush=True)
assert volume>0 and not contacts and not checks
assert all(fingerprint(bpy.data.objects[n])==p for n,p in protected.items())
source=ROOT/'blender/collection/I_nautilus_port_edge_r68.blend';component=ROOT/'app/assets/collection/components/I_nautilus_port_edge_r68.glb';assert not source.exists();bpy.ops.wm.save_as_mainfile(filepath=str(source),compress=True)
...
ART=ROOT/'app/assets/collection/art/I/port_edge_r68';ART.mkdir(parents=True,exist_ok=True);layout=json.loads((ROOT/s['chamber_response_layout'].replace('res://','app/')).read_text());layout.update(source_sha256=sha(source),component_sha256=sha(component));(ART/'chamber_layout.json').write_text(json.dumps(layout,indent=2)+'\n')
d={**s,'source':str(source.relative_to(ROOT)),'source_sha256':sha(source),'component':str(component.relative_to(ROOT)),'component_sha256':sha(component),'chamber_response_layout':'res://assets/collection/art/I/port_edge_r68/chamber_layout.json','port_edge':probe,'status':'envelope_port_liner_candidate'};(OUT/'build.json').write_text(json.dumps(d,indent=2)+'\n')
