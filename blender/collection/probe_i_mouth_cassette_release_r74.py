...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'review/I_refinement/nautilus_r1/mouth_cassette_release_r74';OUT.mkdir(parents=True,exist_ok=True)
...
front=json.loads((ROOT/'review/I_refinement/nautilus_r1/service_r63/stage03_wide/plan.json').read_text());rear=json.loads((ROOT/'review/I_refinement/nautilus_r1/rear03_release_r70/two_stage_r5/probe.json').read_text());assert rear['source_sha256']==s['source_sha256']and all(not x['contacts']for x in rear['samples'])
...
assembly=[o for o in parts if (o.name.startswith('IAM_')or o.name.startswith('IC1_'))and o.name not in released]
...
for phase in ['loosen','extract']:
 for fraction in [i/40 for i in range(41)]:
  d=axis*.006*(fraction if phase=='loosen'else 1.);ad=-axis*.8*fraction if phase=='extract'else Vector();at=BVHTree.FromPolygons([p+ad for p in av],af,all_triangles=True);bt=BVHTree.FromPolygons([p+d for p in bv],bf,all_triangles=True);contacts=[]
...
result={'source_sha256':s['source_sha256'],'assembly_members':[o.name for o in assembly],'bolt_members':[o.name for o in bolts],'coupling_withdrawal':.006,'mouth_offset':list(-axis*.8),'samples':rows,'scope':'Route feasibility only: intact A with both collar halves/seam hardware/bushes, six coupling bolts/washer sets moved .006 before withdrawal. Does not prove actual thread disengagement: original coupling threads are simplified and their engagement span must be designed and validated before animation or acceptance.'};(OUT/'probe.json').write_text(json.dumps(result,indent=2)+'\n')
