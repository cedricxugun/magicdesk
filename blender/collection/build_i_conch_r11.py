...
OUT=ROOT/'review/I_refinement/conch_r11';OUT.mkdir(parents=True,exist_ok=True)
TARGET=ROOT/'blender/collection/I_conch_r11.blend';sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
if TARGET.exists():
...
mouth=b.empty('IH1_Mouth',body,(-.23,-.54,2.02));axis=Vector((.1,1,.02)).normalized();up=Vector((0,0,1));right=up.cross(axis).normalized();vertical=axis.cross(right).normalized();mouth.rotation_mode='QUATERNION';mouth.rotation_quaternion=Matrix((right,vertical,axis)).transposed().to_quaternion()
...
def radius(q):return .025+1.45*max(0.,math.sin(math.pi*q))**.50*(1-.70*q)
...
bpy.context.view_layer.objects.active=b.root;component=ROOT/'app/assets/collection/components/I_conch_r11.glb'
