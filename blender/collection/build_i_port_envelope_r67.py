...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'review/I_refinement/nautilus_r1/port_envelope_r67';OUT.mkdir(parents=True,exist_ok=True)
...
source=ROOT/'blender/collection/I_nautilus_port_envelope_r67.blend';component=ROOT/'app/assets/collection/components/I_nautilus_port_envelope_r67.glb';assert not source.exists();bpy.ops.wm.save_as_mainfile(filepath=str(source),compress=True)
...
ART=ROOT/'app/assets/collection/art/I/port_envelope_r67';ART.mkdir(parents=True,exist_ok=True);layout=json.loads((ROOT/s['chamber_response_layout'].replace('res://','app/')).read_text());layout.update(source_sha256=sha(source),component_sha256=sha(component));(ART/'chamber_layout.json').write_text(json.dumps(layout,indent=2)+'\n')
d={**s,'source':str(source.relative_to(ROOT)),'source_sha256':sha(source),'component':str(component.relative_to(ROOT)),'component_sha256':sha(component),'chamber_response_layout':'res://assets/collection/art/I/port_envelope_r67/chamber_layout.json','port_envelope':probe,'status':'envelope_port_liner_candidate'};(OUT/'build.json').write_text(json.dumps(d,indent=2)+'\n')
