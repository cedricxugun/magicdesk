...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'review/I_refinement/nautilus_r1/shell02_release_r72';OUT.mkdir(parents=True,exist_ok=True)
...
carrier=bpy.data.objects['IN1_PanelPivot_02'];selected=[o for o in [carrier,*carrier.children_recursive]if o.type=='MESH'];assert bpy.data.objects['IN1_PorcelainPanel_02']in selected
...
r={'source_sha256':s['source_sha256'],'prepared_front_and_rear_pose_max_error':pose_error,'prepared_rear_source_sha256':rear['source_sha256'],'selected':[o.name for o in selected],'bounds_blender':bounds,'paths':paths,'scope':'Actual panel02 carrier and its mesh children against all retained actual body, A, parked support/port groups and real base, after the terminal R65 support, R63 front and R70 rear03 poses. Straight translations only; no new animation, physical fastener release, full sequence or art acceptance.'};(OUT/'probe.json').write_text(json.dumps(r,indent=2)+'\n')
