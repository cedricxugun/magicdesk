...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'review/I_refinement/opening_r14';spec=json.loads((OUT/'build.json').read_text());source=ROOT/spec['source'];assert hashlib.sha256(source.read_bytes()).hexdigest()==spec['source_sha256']
...
result={'source_sha256':spec['source_sha256'],'samples':37,'surface_contacts':contacts,'pose_bounds':bounds,'clear_of_all_sampled_contacts':not contacts,'scope':'Moving shell/upper-hood surfaces vs each other, fixed geometry and moving iris at 37 times, including reverse. Includes expected joint-envelope contacts; no silent exemptions. Does not prove contained-volume or continuous clearance, mechanical hardware or art acceptance.'}
