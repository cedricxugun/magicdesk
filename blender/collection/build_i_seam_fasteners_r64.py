...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'review/I_refinement/nautilus_r1/seam_fasteners_r64';OUT.mkdir(parents=True,exist_ok=True)
...
protected={o.name:fingerprint(o)for o in bpy.data.objects if o.type=='MESH'and o.name not in allowed};print('R64_PROTECTED',len(protected),flush=True)
...
for sign in [-1,1]:
    ...
    mesh=bpy.data.meshes.new('IC64_SeamSocketMesh_'+str(sign));mesh.from_pydata(verts,[],faces);mesh.update();cap=bpy.data.objects.new('IC64_SeamSocket_'+str(sign),mesh);pin.users_collection[0].objects.link(cap);cap.parent=mount;mesh.materials.append(pin.data.materials[0])
    ...
    pairs=tree(pin)[0].overlap(tree(cap)[0]);assert not pairs,('Stud/socket overlap',sign,len(pairs))
    records.append({'sign':sign,'pin':pr,'socket':cr,'axis_parent':[0,1,0],'center_parent':[cx,0,cz],'socket_bounds_parent_y':[-.079,-.067],'thread_pitch':.001,'nominal_thread_radial_clearance':.00025,'blind_wall_thickness':.0007,'pin_socket_contacts':len(pairs)})
    print('R64_SEPARABLE_PAIR',sign,pr,cr,flush=True)
assert all(fingerprint(bpy.data.objects[n])==v for n,v in protected.items())
source=ROOT/'blender/collection/I_nautilus_seam_fasteners_r64.blend';component=ROOT/'app/assets/collection/components/I_nautilus_seam_fasteners_r64.glb';assert not source.exists();bpy.ops.wm.save_as_mainfile(filepath=str(source),compress=True)
...
ART=ROOT/'app/assets/collection/art/I/seam_fasteners_r64';ART.mkdir(parents=True,exist_ok=True);layout=json.loads((ROOT/s['chamber_response_layout'].replace('res://','app/')).read_text());layout.update(source_sha256=sha(source),component_sha256=sha(component));(ART/'chamber_layout.json').write_text(json.dumps(layout,indent=2)+'\n')
...
d={**s,'source':str(source.relative_to(ROOT)),'source_sha256':sha(source),'component':str(component.relative_to(ROOT)),'component_sha256':sha(component),'seam_fasteners':proof,'chamber_response_layout':'res://assets/collection/art/I/seam_fasteners_r64/chamber_layout.json','status':'separable_fastener_candidate_checks_pending'}
(OUT/'build.json').write_text(json.dumps(d,indent=2)+'\n');(OUT/'build_check.json').write_text(json.dumps(proof,indent=2)+'\n');print('R64_SAVED',d['source_sha256'],flush=True)
