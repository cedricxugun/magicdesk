...
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'review/I_refinement/nautilus_r1/port_envelope_r67/smooth_r2';OUT.mkdir(parents=True,exist_ok=True)
...
for vertex,p in zip(o.data.vertices,solution['vertices']):vertex.co=M.inverted()@F@Vector(p)
o.data.update();bm=bmesh.new();bm.from_mesh(o.data);bmesh.ops.recalc_face_normals(bm,faces=list(bm.faces));assert all(e.is_manifold for e in bm.edges)and all(v.is_manifold for v in bm.verts);volume=bm.calc_volume(signed=True);bm.to_mesh(o.data);bm.free();o.data.update()
...
source=ROOT/'blender/collection/I_nautilus_port_envelope_r67b.blend';component=ROOT/'app/assets/collection/components/I_nautilus_port_envelope_r67b.glb';assert not source.exists();bpy.ops.wm.save_as_mainfile(filepath=str(source),compress=True)
...
ART=ROOT/'app/assets/collection/art/I/port_envelope_r67b';ART.mkdir(parents=True,exist_ok=True);layout=json.loads((ROOT/s['chamber_response_layout'].replace('res://','app/')).read_text());layout.update(source_sha256=sha(source),component_sha256=sha(component));(ART/'chamber_layout.json').write_text(json.dumps(layout,indent=2)+'\n')
d={**s,'source':str(source.relative_to(ROOT)),'source_sha256':sha(source),'component':str(component.relative_to(ROOT)),'component_sha256':sha(component),'chamber_response_layout':'res://assets/collection/art/I/port_envelope_r67b/chamber_layout.json','port_envelope':probe,'status':'envelope_port_liner_candidate'};(OUT/'build.json').write_text(json.dumps(d,indent=2)+'\n')
