...
## Continuous cached idle steam with angular birth-history advection.
## configure() takes values/resources only. Never pass Nodes or mutable scene objects.
## frame_a/frame_b: Texture3D (or RD RIDs), world_to_anchor: Transform3D,
...
## Additional: birth_angles (PackedFloat32Array, newest first, up to 64),
## history_step (.1 seconds), current_angle, lower_source, valve_source.
## world_to_anchor removes the fixed parent pose, NOT the rotating turntable yaw.
## The nonrigid age warp intentionally never uses occupancy DDA.

const RAY_SHADER=preload("res://idle_steam_raymarch.glsl")
...
func _init()->void:
	...
	_context=StringName("helios_idle_steam_%s"%get_instance_id())
	enabled=true
...
func _notification(what:int)->void:
	if what==NOTIFICATION_PREDELETE:
		...
		if _rd:
			RenderingServer.call_on_render_thread(_free_gpu_resources.bind(_rd,[_ray,_composite,_linear,_nearest,_parameters,_history,_warm_color]))
...
func _render_callback(callback_type:int,render_data:RenderData)->void:
	...
	var angles:PackedFloat32Array=snapshot.get("birth_angles",PackedFloat32Array([0.0]))
	if angles.is_empty():angles=PackedFloat32Array([0.0])
	var angle_count:=mini(64,angles.size())
	var history_step:=maxf(.001,float(snapshot.get("history_step",.1)))
	var history_data:=PackedFloat32Array([angle_count,history_step,float(snapshot.get("current_angle",angles[0])),(angle_count-1)*history_step])
	var lower_source:Vector4=snapshot.get("lower_source",Vector4(.66,1.10,.085,.30))
	...
	var scale:=clampf(float(snapshot.get("resolution_scale",.4)),.25,1.0)
	...
	for view in range(buffers.get_view_count()):
		...
		var world_to_cache:Transform3D=snapshot.get("world_to_anchor",Transform3D.IDENTITY)
		var cache_min:Vector3=snapshot.get("bounds_min",Vector3(-1.55,.5,-1.55))
		var cache_max:Vector3=snapshot.get("bounds_max",Vector3(1.55,4.25,1.55))
		var radius:=Vector2(maxf(absf(cache_min.x),absf(cache_max.x)),maxf(absf(cache_min.z),absf(cache_max.z))).length()
		var all_yaw_bounds:=AABB(Vector3(-radius,cache_min.y,-radius),Vector3(radius*2,cache_max.y-cache_min.y,radius*2))
		var world_bounds:AABB=snapshot.get("world_bounds",world_to_cache.affine_inverse()*all_yaw_bounds)
		...
		data.append_array(PackedFloat32Array([snapshot.get("max_steps",96),snapshot.get("core_shadow_steps",3),0.0,snapshot.get("depth_softness",.09)]))
		_append_vec(data,Vector3(float(snapshot.get("source_off_elapsed",-1.0)),0.0,0.0),0.0)
		...
		var ray_uniforms:Array[RDUniform]=[
			...
			_uniform(7,RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER,[_history])]
		...
		if warming:
			for frame in range(warm_frames.size()):
				...
				ray_uniforms[2]=_uniform(2,RenderingDevice.UNIFORM_TYPE_SAMPLER_WITH_TEXTURE,[_linear,_texture_rd(warm_frames[(frame+1)%warm_frames.size()])])
	...
	if warming:
		...
		print("IDLE_STEAM_GPU_PREWARM ",get_diagnostics())
