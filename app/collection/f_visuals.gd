...
var draw_gain:=0.0
var idle_applied:=false
var calibration_audio:AudioStreamPlayer
var heard_peak_count:=0
var tuning:Dictionary={}
var captured_shards:Array=[]
var captured_glints:Array=[]
var captured_ribbons:Array=[]
const COUNT:=112
const SEGMENTS:=28
...
func setup(owner:Node3D)->void:
	...
	module=owner;instrument=module.play.instrument;tuning=module.data.f_fx.get("tuning",{}).duplicate(true);atlas=load(str(module.data.f_fx.atlas))
	...
	for i in range(COUNT):
		positions.append(Vector3.ZERO);velocities.append(Vector3.ZERO);orientations.append(Quaternion.IDENTITY)
		captured_shards.append({"transform":Transform3D.IDENTITY,"gain":0.,"variant":0.})
		captured_glints.append({"transform":Transform3D.IDENTITY,"gain":0.})
	var ribbon_mesh:=QuadMesh.new();ribbon_mesh.size=Vector2.ONE
	for i in range(3):
		ribbons.append(pool(ribbon_mesh,material(3,true,true),SEGMENTS))
		var pieces:Array=[]
		for j in range(SEGMENTS):pieces.append({"transform":Transform3D.IDENTITY,"gain":0.,"uv":[j/float(SEGMENTS),(j+1)/float(SEGMENTS)]})
		captured_ribbons.append(pieces)
...
func update_ribbon(node:MultiMeshInstance3D,which:int,gain:float)->void:
	...
	for i in range(SEGMENTS):
		...
		var tr:=Transform3D(Basis(x*(b-a).length(),y*.18,z),(a+b)*.5)
		node.multimesh.set_instance_transform(i,tr)
		captured_ribbons[which][i].transform=tr;captured_ribbons[which][i].gain=gain
...
func tick(delta:float,quiet_gain:float)->void:
	if calibration_audio:
		var silent:bool=module.host.muted or module.get_tree().get_meta("collection_no_audio",false)
		if silent:calibration_audio.stop()
		...
		if instrument.peak_count>heard_peak_count:
			...
			if not silent and not force_warm:calibration_audio.play()
	var active:float=module.openness*(1.0-module.explosion)*module.power*quiet_gain
	if force_warm:active=1.0
	if active<=.000001:
		draw_gain=0.
		if not idle_applied:
			for node in [foil,ticks,glints,filament,seed]+receiver_waves+ribbons:node.visible=false
			for node in [filament,seed]+receiver_waves:node.material_override.set_shader_parameter("gain",0.)
			for light in lights:light.light_energy=0.
			for i in range(COUNT):velocities[i]=Vector3.ZERO;orientations[i]=Quaternion.IDENTITY
			initialized=false;idle_applied=true
		for mat in module.instrument_materials:
			mat.set_shader_parameter("instrument_gain",module.power*quiet_gain*.04*float(tuning.get("hardware",1.)))
			mat.set_shader_parameter("instrument_time",module.clock)
		return
	idle_applied=false
	var peak:float=1.0 if force_warm else instrument.peak()
	var crest:float=1.0 if force_warm else instrument.crest()
	var charge:float=instrument.visual_charge
	...
	draw_gain=active*(.35+bias*1.8+charge*.8+peak*2.2+crest*1.4)*idle*float(tuning.get("field_gain",1.0))
	...
	for i in range(COUNT):
		...
		var solid_transform:=Transform3D(basis,positions[i]);var solid_gain:=intensity;var variant:=.45+.45*sin(float(i)*3.7)
		if i<28:
			solid_transform=Transform3D(basis*Basis.from_scale(Vector3(1.2,1.2,1.2)),positions[i]);solid_gain=intensity*.85;variant=.65
			ticks.multimesh.set_instance_transform(i,solid_transform);ticks.multimesh.set_instance_custom_data(i,Color(solid_gain,variant,0,0))
		captured_shards[i].transform=solid_transform;captured_shards[i].gain=solid_gain;captured_shards[i].variant=variant
		var glint_transform:=Transform3D(camera*Basis.from_scale(Vector3.ONE*scale*(.4 if i>=28 else .8)),positions[i])
		var glint_gain:=intensity*.55*float(tuning.get("glints",1.0))
		glints.multimesh.set_instance_transform(i,glint_transform);glints.multimesh.set_instance_custom_data(i,Color(glint_gain,0,0,0))
		captured_glints[i].transform=glint_transform;captured_glints[i].gain=glint_gain
	initialized=true;foil.visible=active>.01;ticks.visible=active>.01;glints.visible=draw_gain>.025
	for i in range(3):update_ribbon(ribbons[i],i,active*(.08+bias*.8+charge*.30+peak*1.7+release*.6)*idle*float(tuning.get("ribbons",1.0)))
	...
	filament.material_override.set_shader_parameter("gain",focus*float(tuning.get("plumb_beam",1.0)))
	for i in range(receiver_waves.size()):
		...
		wave.material_override.set_shader_parameter("gain",active*peak*(.65+crest)*(1.0-phase)*float(tuning.get("receiver",1.0)));wave.visible=active*peak>.005
	...
	seed.material_override.set_shader_parameter("gain",active*(release+peak*(1.0-smoothstep(.92,1.0,returning)))*float(tuning.get("seed",1.0)));seed.visible=active*(release+peak)>.01
	for material in module.instrument_materials:
		material.set_shader_parameter("instrument_gain",module.power*quiet_gain*(.04+active*(.30+bias*.55+charge+.8*peak+crest*2+release))*float(tuning.get("hardware",1.0)))
	...
	for light in lights:light.light_energy*=float(tuning.get("lights",1.0))

func state(full:bool=false)->Dictionary:
	var shards:Array=[]
	for i in range(COUNT):
		var node:MultiMeshInstance3D=ticks if i<28 else foil
		var tr:Transform3D=captured_shards[i].transform
		var q:=tr.basis.get_rotation_quaternion();var s:=tr.basis.get_scale();var p:=tr.origin
		shards.append({"p":[p.x,p.y,p.z],"q":[q.x,q.y,q.z,q.w],"s":[s.x,s.y,s.z],"glow":captured_shards[i].gain,"variant":captured_shards[i].variant,"kind":"tick" if i<28 else "swarf"})
	var result:={"capture_source":"CPU values written to MultiMesh; valid with dummy renderer","solid_foil_instances":COUNT,"allocated_once":true,"gain":draw_gain,"visible":foil.visible,"stage":instrument.stage,"peak_time":instrument.peak_time,"shards":shards}

	if full:
		result.clock=module.clock;result.tuning=tuning.duplicate(true)
		result.glints=[];result.ribbons=[];result.waves=[];result.lights=[]
		for i in range(COUNT):result.glints.append({"pose":_pack(captured_glints[i].transform),"gain":captured_glints[i].gain,"visible":glints.visible})
		for which in range(ribbons.size()):
			var ribbon:MultiMeshInstance3D=ribbons[which]
			var pieces:Array=[]
			for i in range(SEGMENTS):
				var record:Dictionary=captured_ribbons[which][i]
				pieces.append({"pose":_pack(record.transform),"gain":record.gain,"uv":record.uv,"visible":ribbon.visible})
			result.ribbons.append(pieces)
		result.filament=_plane(filament);result.seed=_plane(seed)
		for wave in receiver_waves:result.waves.append(_plane(wave))
		for light in lights:result.lights.append({"position":[light.global_position.x,light.global_position.y,light.global_position.z],"energy":light.light_energy,"color":[light.light_color.r,light.light_color.g,light.light_color.b],"range":light.omni_range})
		result.hardware_gain=float(module.instrument_materials[0].get_shader_parameter("instrument_gain")) if not module.instrument_materials.is_empty() else 0.
	return result

func _pack(t:Transform3D)->Dictionary:
...
func _plane(node:MeshInstance3D)->Dictionary:
	return {"pose":_pack(node.global_transform),"size":[node.mesh.size.x,node.mesh.size.y],"gain":float(node.material_override.get_shader_parameter("gain")),"visible":node.visible}

func _exit_tree()->void:
	if calibration_audio:
		calibration_audio.stop();calibration_audio.stream=null
